		...
		try {
			if (isset($_SERVER['HTTP_X_WP_AUTHORIZATION'])) {
				...
				if (!$jwtUser || !$jwtUser->exists()) {
					...
					// jwt user exists need just to login but checck if the any user logged
...
//		  echo $e;
	...
	public function createUser($user_email)
	{
		...
		// wp_die( __( 'The domain or path entered conflicts with an existing username.'.$user_id ) );
		if (!$user_id && false == email_exists($user_email)) {
//			print_r("createUser yes");
//   wp_die( __( 'inside  createUser yesss create '.$user_id ) );
...
//			print_r("createUser no");
//     wp_die( __( 'inside  createUser else no create   '.$user_id ) );
		}
